...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="mb-6 flex justify-between items-center">
            <form action="{{ route('articulos.index') }}" method="GET" class="flex items-center space-x-2">
            ...
            @if (auth()->user()->rol === 'admin' || auth()->user()->rol === 'supervisor')
                <x-button onclick="toggleModal('addProductoModal')"
    ...
    <div id="addProductoModal" class="fixed inset-0 z-50 flexitems-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-6 w-full max-w-lg">
            ...
            <form action="{{ route('articulos.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex justify-end">
                    <x-button type="button" class="btn btn-secondary mr-2"
                        onclick="toggleModal('addProductoModal')">Cancelar</x-button>
                    <x-button type="submit"
                        class="btn btn-primary bg-blue-600 text-white px-4 py-2 rounded-lg">Guardar</x-button>
                </div>
            </form>
        </div>
    </div>
